...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps):
    ...
    tower_gradients = []
    for l in loss:
        gradients = optimizer.compute_gradients(l, colocate_gradients_with_ops=False)
        tower_gradients.append(gradients)

    merged_gradients = combine_gradients(tower_gradients)

    merged_gradients = clip_gradient_norms(merged_gradients, max_norm=1.0)
    ...
    train_op = tf.group(train_op, [global_step.assign(new_global_step), update_ops])
    return train_op
