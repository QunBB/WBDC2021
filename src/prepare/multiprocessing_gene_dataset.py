...
from src.prepare.utils import DataInstance, get_action_df, multiprocess_write, write_example
...
def run(csv_file, feed_csv_file, maps_file, dataset_dir, pool_num, do_eval=True, k_fold=None,
    ...
    if os.path.exists(os.path.join(dataset_dir, dataset_path)) and len(
        ...
        print(os.listdir(os.path.join(dataset_dir, dataset_path)))
    ...
    if do_eval:
        writer_list = [tf.python_io.TFRecordWriter(os.path.join(tfrecord_dir, "eval.tfrecord"))]
        tfrecord_dir = os.path.join(tfrecord_dir, "do_eval")
    else:
        writer_list = []
        tfrecord_dir = os.path.join(tfrecord_dir, "all_in")

    if k_fold is None:
        os.system("mkdir -p {}".format(os.path.join(tfrecord_dir, "no_kfold/1")))
        writer_list.extend([
            tf.python_io.TFRecordWriter(os.path.join(tfrecord_dir, "no_kfold/1/{}.tfrecord".format(i)))
            for i in range(pool_num)])
    else:
        for k in range(k_fold):
            os.system("mkdir -p {}".format(os.path.join(tfrecord_dir, "kfold/{}".format(k))))
            writer_list.extend(
                [tf.python_io.TFRecordWriter(os.path.join(tfrecord_dir, "kfold/{}/{}.tfrecord".format(k, i)))
                 for i in range(pool_num)])
    ...
    for name in os.listdir(os.path.join(dataset_dir, dataset_path)):
        with open(os.path.join(dataset_dir, dataset_path, name), "rb") as file:
            dataset = list(chain(*pickle.load(file)))
        ...
        if do_eval:
            examples = [[d for d in dataset if d.date == 14]]
            dataset = [d for d in dataset if d.date < 14]
            eval_num += len(examples[0])
        else:
            examples = []

        if mask_rate is not None:
            def _mask(x):
                # TODO 此处应设为测试集冷启动feedid
                x.feedid_origin = 0
                return x
            # 选取当天第一次出现的feed样本
            mask_data = [d for d in dataset if d.date == d.first_date]
            mask_data = random.sample(mask_data, min(int(len(dataset) * mask_rate), len(mask_data)))
            mask_data = list(map(lambda x: _mask(x), mask_data))

            print("The number of feed-mask data is: {}".format(len(mask_data)))
        ...
        if k_fold is None:
            one_fold_num += len(dataset)
            linspace = np.linspace(0, len(dataset), pool_num + 1, dtype=int)
            examples.extend([dataset[linspace[i]:linspace[i + 1]] for i in range(pool_num)])
        else:
            positive_data = list(
                filter(lambda
            ...
            negative_data = list(
                filter(lambda
                       ...
                       dataset))
            one_fold_num += len(positive_data) + len(negative_data) // k_fold
            fold_linspace = np.linspace(0, len(negative_data), k_fold + 1, dtype=int)
            for f in range(k_fold):
                dataset = positive_data + negative_data[fold_linspace[f]:fold_linspace[f + 1]]
                random.shuffle(dataset)
                linspace = np.linspace(0, len(dataset), pool_num + 1, dtype=int)
                examples.extend([dataset[linspace[i]:linspace[i + 1]] for i in range(pool_num)])
        assert len(writer_list) == len(examples)

        pool = Pool(pool_num)
        result = pool.map(get_example, examples)
        pool.close()
        pool.join()

        for i, (ret, writer) in enumerate(zip(result, writer_list)):
            print("Writer ", i)
            for r in tqdm(ret, total=len(ret)):
                writer.write(r)
    ...
    print("do_eval: {}, eval_num: {}".format(do_eval, eval_num))
    print("The number of 1-fold dataset is: {}".format(one_fold_num))
    print("The number of all dataset is: {}".format(all_num))
    ...
    for num in user_num:
        ...
        for index in set(data.index):
            ...
            for feedid_map, device, read_comment, comment, like, click_avatar, \
                forward, follow, favorite, feedid_origin, uid, userid_map, first_date in \
                    ctr[
                        ['feedid_map', 'device', 'read_comment', 'comment', 'like', 'click_avatar',
                         'forward', 'follow', 'favorite', 'feedid', 'userid', 'userid_map', 'first_date']].values:
...
def get_example(data):
    ...
    for instance in data:
        example = write_example(instance).SerializeToString()
